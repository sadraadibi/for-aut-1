...
void move (int map [yyyyyyyy][xxxxxxxx]) {
	...
	switch(key) {
		case 72: 			if (pacmany > 0 && map[pacmany-1][pacmanx] && map[pacmany-1][pacmanx]!=8)pacmany--;
				map[pacmany][pacmanx] = 2;
			break;;//UP 
		case 80: 			if ( map[pacmany+1][pacmanx] && map[pacmany+1][pacmanx]!=8)pacmany++;
				map[pacmany][pacmanx] = 2;
			break;//DOWN 
		case 75: 		    if (pacmanx > 0 && map[pacmany][pacmanx-1] && map[pacmany][pacmanx-1]!=8)pacmanx--;
				map[pacmany][pacmanx] = 2;
			break;//LEFT
		case 77: 			if ( map[pacmany][pacmanx+1] && map[pacmany][pacmanx+1]!=8)pacmanx++;
				map[pacmany][pacmanx] = 2;	
			break;//RIGHT
